...
```{r}
mains <- list(
  USACanada = "USA / Canada", Europe = "Europe", NorthChina = "NorthChina",
...
axlabs <- list(
  USACanada = list(x = seq(0, 14, by = 1), labs = 2012 - seq(0, 14, by = 1)),
...
{
  results[[region]] <- list(
    bnpr = BNPR(data = regional_flu[[region]], lengthout = 100),
...
for (region in regions)
{
  ...
  plot_mrw(
    BNPR_outs = results[[region]], axlabs = axlabs[[region]], main = "",
