...
```{r results='hide'}
res_MCMC <- mcmc_sampling(
  data = gene, alg = "ESS", nsamp = nsamp,
...
```{r}
coal_ppc <- posterior_coal_check(
  coal_times = res_MCMC$coal_times,
...
```{r fig.width=6, fig.height=6, out.width='50%'}
plot(coal_ppc$obs, coal_ppc$rep,
...
```{r fig.width=6, fig.height=6, out.width='50%'}
plot(samp_ppc$obs, samp_ppc$rep,
  pch = 16, col = rgb(0, 0, 0, 0.5),
