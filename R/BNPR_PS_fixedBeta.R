infer_coal_samp_fixed <- function(samp_times, coal_times, n_sampled = NULL, fns = NULL,
  ...
  if (simplify) {
    coal_data <- with(coal_data, condense_stats(
      time = time,
      event = event, E = E
    ))
  }
  ...
  if (!use_samp) {
    data <- with(coal_data, data.frame(
      y = event, time = time,
      E_log = E_log
    ))
    formula <- y ~ -1 + f(time,
      model = "rw1", hyper = hyper,
      constr = FALSE
    )
    family <- "poisson"
  } else if (use_samp) {
...
BNPR_fixed <- function(data, lengthout = 100, pref = TRUE, prec_alpha = 0.01,
  ...
  result$summary <- with(
    ...
    data.frame(
      time = ID, mean = exp(-mean), sd = sd * exp(-mean),
  ...
  if (pref) {
    result$beta0 <- result$result$summary.fixed[
      "beta0",
      "0.5quant"
    ]
