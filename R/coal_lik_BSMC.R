...
# TODO: Think about changing the ocurrances at the exact time and instead at the defined interval
{
  ...
  for (j in 2:(ntrees)) {
    ...
    if (init$latent[j] == 1) {
      ...
      N1[1] <- 0
      N2 <- (1 - (1 - logq) * q) * ((exp(f.ext) / init$C[j, ])^{
        2
      }) * init$I[j, ]
      N3 <- ((exp(f.ext) / init$C[j, ])^{
        2
      }) * (1 / init$C[j, ]) * (-logq - 2 + (2 - logq) * q)
      ...
      if (length(indic) > 0) {
        if (min(indic) > m.0) {
          ExpDelta[j, indic] <- init$Delta[j, indic]
        }
      }
    ...
    if (latent[j] == 0) {
      ...
      if (min(children) == 2) {
        ...
        if (min(children) == max(children) & max(children) == 1) {
          ...
          val0_new <- 0
          if (min(children) == 0) {
            val0 <- max(seq(1, length(children), by = 1)[children == 0])
          }
          val <- max(seq(1, length(children), by = 1)[children == 1])
          if (min(children_new) == 0) {
            val0_new <- max(seq(1, length(children_new), by = 1)[children_new == 0])
          }
          if (min(children_new) <= 1) {
            val_new <- max(seq(1, length(children_new), by = 1)[children_new == 1])
          } else {
            val_new <- 0
          }
          if (val0 > 0 & val0_new > 0) {
            if (((D[j - 1, val0] == D[j, val0_new]) & (D[j - 1, val] == D[j, val_new])) || (sum(children - children_new) == 0) & (val_new < where_new[j])) {
              ...
              if (val0_new < val0) {
                info_times[j, 1:3] <- c(1, D[j, val_new], min(t_del[j], t_new[j]))
              }
              if (val0_new == val0) {
                info_times[j, 1:3] <- c(1, D[j, val0_new], min(t_del[j], t_new[j]))
              }
              if (val0_new > val0) {
                info_times[j, 1:3] <- c(1, D[j, val], min(t_del[j], t_new[j]))
              }
            }
          } else {
            if (val0_new < val0) {
            ...
            if (val0_new == val0 & val0_new > 0) {
              info_times[j, 1:3] <- c(1, D[j, val0_new], min(t_del[j], t_new[j]))
            }
            if (val0_new == val0 & val0_new == 0 & val_new == val & val < where_new[j]) {
              info_times[j, ] <- c(1, 0, D[j, val], 2, D[j, val], min(t_del[j], t_new[j]))
            }
            if (val0_new > val0 & val0_new == val) {
              info_times[j, 1:3] <- c(1, D[j, val], min(t_del[j], t_new[j]))
            }
            if (val0_new > val0 & val_new == val) {
              info_times[j, 1:3] <- c(1, D[j, val], min(t_del[j], t_new[j]))
            }
            if (val0_new == val0 & val0_new == 0 & val_new != val) {
              info_times[j, 1:3] <- c(1, 0, min(t_del[j], t_new[j]))
            }
          }
        }
