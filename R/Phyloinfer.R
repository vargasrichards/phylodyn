...
initialize_params <- function(samp_times, n_sampled, coal_times, grid,
  ...
  return(list(
    lik_init = lik_init, prec_alpha = prec_alpha, prec_beta = prec_beta,
...
compute_pos_summ2 <- function(samp_alg, loglikf, f, prec, first_elem_prec,
  ...
  if (samp_alg == "MH" || samp_alg == "fixed") {
    loglik <- ESS_betas_ll2(
      f = f, lik_init = lik_init, betas = betas,
  ...
  if (samp_alg == "MH") {
    logbetapri <- log_betas_prior2(
      betas = c(betas, covar_betas, pow_covar_betas),
...
Q_matrix <- function(input, s_noise = 0, signal = 1) {
  ...
  Q <- spam::spam(0, n2, n2)
  if (n2 > 2) {
    Q[cbind(seq(1, n2), seq(1, n2))] <- c(
      diff[1], diff[1:(n2 - 2)] + diff[2:(n2 - 1)],
      diff[n2 - 1]
    ) + (1 / signal) * rep(s_noise, n2)
  } else {
...
calculate_estimates <- function(logfmat, params, grid) {
  ...
  return(list(
    logfmed = logfmed, logflow = logflow, logfhi = logfhi,
...
MH_betas <- function(curr_betas, curr_pos_summ, loglikf, lik_init, f, kappa,
  ...
  new_pos_summ <- compute_pos_summ(
    samp_alg = "MH", loglikf = loglikf, f = f,
...
MH_betas_rscan2 <- function(curr_betas, curr_pos_summ, loglikf, lik_init, f, prec,
  ...
  for (idx in rscan)
  {
    ...
    new_pos_summ <- compute_pos_summ2(
      samp_alg = "MH", loglikf = loglikf, f = f,
...
sampling <- function(data, para, alg, setting, init, verbose = TRUE, printevery = 100) {
  ...
  if (alg == "HMC") {
    Ufun <- function(theta, grad = FALSE) {
      U(
        theta = theta, init = lik_init, invC = invC,
...
ESS_betas_ll2 <- function(f, lik_init, betas, covar_vals = NULL, covar_betas = NULL,
  ...
  return(coal_loglik(init = lik_init, f = f) +
    samp_loglik_pow(
      init = lik_init, logpop = f, betas = betas,
...
sampling_ESS <- function(data, para, setting, init,
  ...
  pos_summ <- compute_pos_summ(
    samp_alg = samp_alg, loglikf = ll, f = f,
    kappa = kappa, invC = invC, lik_init = lik_init,
  ...
  for (iter in 1:NSAMP)
  {
    if (samp_alg == "none") {
      res <- ESS(
        q_cur = f, l_cur = pos_summ$loglik, loglik = ll,
      ...
      MH_res <- MH_betas_rscan(
        curr_betas = betas, curr_pos_summ = pos_summ,
      ...
      res <- ESS(
        q_cur = c(f, betas), l_cur = pos_summ$loglik, loglik = ll,
    ...
    if (kappa_alg == "gibbs") {
      ...
      kappa_res <- whiten_kappa(
        kappa = kappa, f = f, lik_init = lik_init,
  ...
  return(list(
    samp = samp, alg = "ESS", time = time, pos_summ = pos_summ, pos_summ_out = pos_summ_out,
...
sampling_ESS2 <- function(data, para, setting, init,
  ...
  for (iter in 1:NSAMP)
  {
    if (samp_alg == "none") {
      res <- ESS2(
        q_cur = f, l_cur = pos_summ$loglik, loglik = ll, prec = prec,
  ...
  return(list(
    samp = samp, alg = "ESS2", time = time, pos_summ = pos_summ_out,
...
mcmc_sampling <- function(dataset, alg, nsamp, nburnin = 0, nsubsamp = 1, ngrid = 100,
  ...
  if (nugget == "1,1") {
    ...
    diag(invC) <- diag(invC) + .0001
  } # nugget for the whole diagonal
  else if (nugget == "none") {
  ...
  para <- list(
    alpha = prec_alpha, beta = prec_beta, invC = invC, rtEV = rtEV,
  ...
  # Run MCMC sampler
  if (alg == "ESS") {
    res_MCMC <- sampling_ESS(
      data = dataset, para = para, setting = setting,
      init = init, samp_alg = samp_alg, kappa_alg = kappa_alg,
      printevery = printevery
    )
  } else if (alg == "ESS2") {
    res_MCMC <- sampling_ESS2(
      data = dataset, para = para, setting = setting,
      init = init, samp_alg = samp_alg, kappa_alg = kappa_alg,
      printevery = printevery
    )
    ...
    res_MCMC <- sampling(
      data = dataset, para = para, alg = alg, setting = setting,
