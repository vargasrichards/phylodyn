...
plot_seasonality <- function(BNPR_out, zero_date, start = 0.0, years = NULL,
  ...
  graphics::lines(
    x = seq(0, 1, length.out = 101), y = fun_mean,
...
plot_BNPR_old <- function(BNPR_out, traj = NULL, xlim = NULL, ...) {
  ...
  graphics::plot(grid, exp(-mod$"0.5quant"),
    type = "l", lwd = 2.5, col = "blue", log = "y",
    xlab = "Time (past to present)", ylab = "Scaled Effective Pop. Size",
    xlim = xlim, ylim = c(
      min(exp(-mod$"0.975quant"[grid > min(xlim) & grid < max(xlim)])),
...
plot_INLA_inv <- function(BNPR_out, traj = NULL, xlim = NULL, ...) {
  ...
  graphics::plot(grid, exp(mod$"0.5quant"),
    ...
    xlab = "Time (past to present)", ylab = "Scaled Effective Pop. Size",
    xlim = xlim, ylim = c(
      min(exp(mod$"0.025quant"[grid > min(xlim) & grid < max(xlim)])),
...
plot_BEAST <- function(midpts, med, up, low, cutoff = NULL, ylim = NULL,
  ...
  if (!is.null(samp_times)) {
    ...
    graphics::text(
      x = lab_x, y = ylim[1] * yextra, labels = "Sampling events",
...
plot_seasonality_beast <- function(BEAST_out, zero_date, start = 0.0, years = NULL,
  ...
  graphics::lines(
    x = seq(0, 1, length.out = 101), y = fun_median,
  ...
  if (heatmaps) {
    ...
    if (heatmap_labels) {
      ...
      if (log_y) {
        graphics::text(
          x = lab_x, y = ymin / (yextra^0.0), labels = "Sampling events",
