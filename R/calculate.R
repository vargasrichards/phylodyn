...
BNPR <- function(data, lengthout = 100, pref = FALSE, prec_alpha = 0.01,
  ...
  if (derivative) {
    ...
    result$derivative <- with(
      ...
      data.frame(
        time = result$x, mean = -mean[ind], sd = sd[ind],
...
infer_coal <- function(samp_times, coal_times, n_sampled = NULL, lengthout = 100,
  ...
  coal_data <- coal_stats(
    grid = grid, samp_times = samp_times, n_sampled = n_sampled,
    coal_times = coal_times
  )
  ...
  if (derivative) {
    ...
    mod <- INLA::inla(formula,
      family = "poisson", data = data, lincomb = lc_many,
      control.predictor = list(compute = TRUE),
      control.inla = list(lincomb.derived.only = FALSE)
    )
  } else {
    mod <- INLA::inla(formula,
      family = "poisson", data = data, offset = data$E_log,
      control.predictor = list(compute = TRUE)
    )
  }
  ...
  if (!use_samp) {
    ...
    if (events_only) {
      ...
      samp_data <- samp_stats(
        grid = grid, samp_times = samp_times,
        n_sampled = n_sampled
      )
    }
...
infer_samp_exper <- function(samp_times, fns, n_sampled = NULL, lengthout = 100,
  ...
  mod <- INLA::inla(formula_sampling,
...
infer_coal_samp_exper <- function(samp_times, coal_times, n_sampled = NULL, fns = NULL,
  ...
  mod <- INLA::inla(formula,
    family = family, data = data, offset = data$E_log,
    control.predictor = list(compute = TRUE),
    control.inla = list(lincomb.derived.only = FALSE)
  )
...
BNPR_samp_only <- function(data, lengthout = 100, grid = NULL, tmrca = NULL,
  ...
  result <- list(
    q025Samp = exp(quants$`0.025quant` + beta0),
...
log_samp_mat <- function(popTbl, betaTbl, covariates = NULL,
  ...
  for (i in 1:n) {
    ...
    samp_mat[i, ] <- log_samp_int(
      logpop = logpop, betas = betas,
