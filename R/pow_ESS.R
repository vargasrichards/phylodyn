...
ESS_betas_ll3 <- function(f, lik_init, betas_list, covars_list = NULL) {
  return(coal_loglik(init = lik_init, f = f) +
    samp_loglik3(
      init = lik_init, logpop = f, betas_list = betas_list,
...
compute_pos_summ3 <- function(f, prec, hyperparams, lik_init, setting,
  ...
  if (setting$samp_alg == "MH" || setting$samp_alg == "fixed") {
    loglik <- ESS_betas_ll3(
      f = f, lik_init = lik_init, betas_list = betas_list,
      covars_list = covars_list
    )
  } else {
...
MH_betas_rscan3 <- function(curr_betas_list, curr_pos_summ, lik_init, f, prec,
  ...
  curr_betas <- c(
    curr_betas_list$betas, curr_betas_list$covar_betas,
  ...
  for (idx in rscan)
  {
    ...
    new_pos_summ <- compute_pos_summ3(
      f = f, prec = prec, hyperparams = hyperparams,
...
sampling_ESS3 <- function(data, para, setting, init,
  ...
  if (samp_alg == "none") {
    ...
    betas_list <- list(
      betas = init$betas, covar_betas = init$covar_betas,
  ...
  if (samp_alg == "MH") {
    betas_out <- matrix(NA,
      nrow = length(recorded_iters),
      ncol = 2 + length(betas_list$covar_betas) + length(betas_list$pow_covar_betas)
    )
  }

  pos_summ <- compute_pos_summ3(
    f = f, prec = prec, hyperparams = para,
    lik_init = lik_init, setting = setting,
  ...
  for (iter in 1:NSAMP)
  {
    if (samp_alg == "none") {
      ...
      MH_res <- MH_betas_rscan3(
        curr_betas_list = betas_list,
    ...
    if (!is.na(output_index)) {
      ...
      if (samp_alg == "MH") {
        betas_out[output_index, ] <- c(
          betas_list$betas, betas_list$covar_betas,
          betas_list$pow_covar_betas
        )
      }
  ...
  return(list(
    latent = fmat, params = params_out, precs = precs, samp = samp,
...
mcmc_ESS <- function(dataset, nsamp, nburnin = 0, nsubsamp = 1, ngrid = 100, printevery = 100,
  ...
  para <- list(
    alpha = prec_alpha, beta = prec_beta, beta_prec = beta_prec,
  ...
  init <- list(
    f = f_init, prec = prec, betas = betas,
