...
{
  y <- exp(log_sampling_intensity(
    logpop = logpop, beta0 = beta0, beta1 = beta1,
...
samp_check_exp <- function(samp_times, n_sampled, logpop, grid, betas) {
  ...
  samp_rep <- c(minsamp, pref_sample(
    f = traj, lim = c(minsamp, maxsamp),
    c = exp(betas[1]), beta = betas[2]
  ), maxsamp)
  exps_rep <- samp_to_exp(
    samp_times = samp_rep, n_sampled = rep(1, length(samp_rep)),
    logpop = logpop, grid = grid,
    beta0 = betas[1], beta1 = betas[2]
  )
...
samp_check_exp2 <- function(samp_times, n_sampled, logpop, grid, betas,
  ...
  log_int <- log_sampling_intensity(
    logpop = logpop, beta0 = betas[1], beta1 = betas[2],
    covariates = covariates, cov_betas = cov_betas,
    pow_covariates = pow_covariates, pow_cov_betas = pow_cov_betas
  )
...
coal_check <- function(coal_times, samp_times, n_sampled, logpop, grid) {
  exps_obs <- coal_to_exp(
    coal_times = coal_times, samp_times = samp_times,
  ...
  exps_rep <- coal_to_exp(
    coal_times = coal_rep$coal_times, samp_times = samp_times,
    n_sampled = n_sampled, logpop = logpop,
    grid = grid
  )
  KSrep <- exp_to_KS(exps_rep)$statistic
...
posterior_coal_check <- function(coal_times, samp_times, n_sampled, logpop, grid, cap = NULL) {
  ...
  for (i in first:niter)
  {
    ...
    result[i - first + 1, ] <- coal_check(
      coal_times = coal_vec, samp_times = samp_times,
